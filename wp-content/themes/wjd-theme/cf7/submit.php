...
add_action( 'wpcf7_init', 'wjd_wpcf7_add_form_tag_submit', 5, 0 );

function wjd_wpcf7_add_form_tag_submit() {
	wpcf7_add_form_tag( 'submit', 'wjd_wpcf7_submit_form_tag_handler' );
}

function wjd_wpcf7_submit_form_tag_handler( $tag ) {
	$class = wpcf7_form_controls_class( $tag->type );

	$atts = array();

	$atts['class'] = $tag->get_class_option( $class ).' webform-button--submit button button--primary js-form-submit form-submit';
	...
	$html = sprintf( '<div class="form-actions form-wrapper"><input %1$s /></div>', $atts );
...
add_action( 'wpcf7_admin_init', 'wjd_wpcf7_add_tag_generator_submit', 45 );

function wjd_wpcf7_add_tag_generator_submit() {
	$tag_generator = WPCF7_TagGenerator::get_instance();
	$tag_generator->add( 'submit', __( 'submit', 'contact-form-7' ),
		'wpcf7_tag_generator_submit', array( 'nameless' => 1 ) );
}

function wjd_wpcf7_tag_generator_submit( $contact_form, $args = '' ) {
