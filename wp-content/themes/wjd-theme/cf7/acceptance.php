...
add_action( 'wpcf7_init', 'wjd_wpcf7_add_form_tag_acceptance', 5, 0 );

function wjd_wpcf7_add_form_tag_acceptance() {
	wpcf7_add_form_tag( 'acceptance',
		'wjd_wpcf7_acceptance_form_tag_handler',
		array(
			'name-attr' => true,
		)
	);
...
function wjd_wpcf7_acceptance_form_tag_handler( $tag ) {
	if ( empty( $tag->name ) ) {
		return '';
	}
    ...
    if (empty($item_atts['id'])) {
        $item_atts['id'] = 'accept_id';
    }
    $item_id = $item_atts['id'];
	...
	if ( $content ) {
		...
		$html = sprintf(
			'%1$s<label for="%3$s" class="option js-form-required form-required">%2$s</label>',
			$html, $content, $item_id
		);
...
add_filter( 'wpcf7_validate_acceptance',
	'wjd_wpcf7_acceptance_validation_filter', 5, 2 );

function wjd_wpcf7_acceptance_validation_filter( $result, $tag ) {
	if ( ! wjd_wpcf7_acceptance_as_validation() ) {
		return $result;
	}
...
add_filter( 'wpcf7_acceptance', 'wjd_wpcf7_acceptance_filter', 5, 2 );

function wjd_wpcf7_acceptance_filter( $accepted, $submission ) {
...
add_filter( 'wpcf7_form_class_attr',
	'wjd_wpcf7_acceptance_form_class_attr', 5, 1 );

function wjd_wpcf7_acceptance_form_class_attr( $class ) {
	if ( wjd_wpcf7_acceptance_as_validation() ) {
...
function wjd_wpcf7_acceptance_as_validation() {
...
add_filter( 'wpcf7_mail_tag_replaced_acceptance',
	'wjd_wpcf7_acceptance_mail_tag', 5, 4 );

function wjd_wpcf7_acceptance_mail_tag( $replaced, $submitted, $html, $mail_tag ) {
...
add_action( 'wpcf7_admin_init', 'wjd_wpcf7_add_tag_generator_acceptance', 25, 0 );

function wjd_wpcf7_add_tag_generator_acceptance() {
	$tag_generator = WPCF7_TagGenerator::get_instance();
	$tag_generator->add( 'acceptance', __( 'acceptance', 'contact-form-7' ),
		'wjd_wpcf7_tag_generator_acceptance' );
}

function wjd_wpcf7_tag_generator_acceptance( $contact_form, $args = '' ) {
