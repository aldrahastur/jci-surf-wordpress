            ...
            if (post.featured_media > 0) {
                new wp.api.models.Media({id:post.featured_media}).fetch().then((image) => {
                    ...
                    this.props.setAttributes( {
                        ...
                        content: post.content.rendered.replace( /[\r\n]+/gm, "" ).replace(/(<div class="card__image-credit"([^<]+)<\/div>)/ig, "").replace(/(<([^>]+)>)/ig, '').slice(0,460)+'...',
                        link: post.link,
                        date: post.date.slice(8, 10) + '.' + post.date.slice(5, 7) + '.' + post.date.slice(0, 4)
                    });
                    new wp.api.models.Category({id:post.categories[0]}).fetch().then((cat) => {
                        this.props.attributes.selectedCategory = cat.name;
                    })
    ...
    render() {
        ...
        if( this.state.posts.length > 0 ) {
            ...
            this.state.posts.forEach((post) => {
                options.push({value:post.id, label:post.title.rendered.replace(/&#8222;/g, '').replace(/&#8220;/g, '')});
            });
        } else {
...
registerBlockType( 'wjd/postcard', {
    title: __( 'Beitrags-Karte' ),
    icon: 'editor-table',
    category: 'common',
    keywords: [
        'card',
