...
add_action( 'wpcf7_init', 'wjd_wpcf7_add_form_tag_checkbox', 5, 0 );

function wjd_wpcf7_add_form_tag_checkbox() {
	wpcf7_add_form_tag( array( 'checkbox', 'checkbox*', 'radio' ),
		'wjd_wpcf7_checkbox_form_tag_handler',
		array(
			'name-attr' => true,
...
function wjd_wpcf7_checkbox_form_tag_handler( $tag ) {
	if ( empty( $tag->name ) ) {
		return '';
	}
	...
	foreach ( $values as $key => $value ) {
		...
		$item_atts = array(
            ...
            'id' => $tag->name . ($multiple ? '[]' : '').$key,
            'required' => 'required'
        );
        $for = $item_atts['name'].$key;
		$item_atts = wpcf7_format_atts( $item_atts );

		$item = sprintf(
            '<input %2$s /><label for="'.$for.'" class="wpcf7-list-item-label">%1$s</label>',
		...
		$class = 'form-item js-form-type-radio';
		$count += 1;
    ...
    $atts['class'] = 'js-webform-radios';
    $atts = wpcf7_format_atts( $atts );

	$html = sprintf(
		'<fieldset class="form-item form-wrapper %1$s"><div %2$s>%3$s</div>%4$s</fieldset>',
...
add_filter( 'wpcf7_validate_checkbox',
	'wjd_wpcf7_checkbox_validation_filter', 5, 2 );
add_filter( 'wpcf7_validate_checkbox*',
	'wjd_wpcf7_checkbox_validation_filter', 5, 2 );
add_filter( 'wpcf7_validate_radio',
	'wjd_wpcf7_checkbox_validation_filter', 5, 2 );

function wjd_wpcf7_checkbox_validation_filter( $result, $tag ) {
	$name = $tag->name;
...
add_filter( 'wpcf7_posted_data', 'wjd_wpcf7_checkbox_posted_data', 10, 1 );

function wjd_wpcf7_checkbox_posted_data( $posted_data ) {
...
add_action( 'wpcf7_admin_init',
	'wjd_wpcf7_add_tag_generator_checkbox_and_radio', 20, 0 );

function wjd_wpcf7_add_tag_generator_checkbox_and_radio() {
	$tag_generator = WPCF7_TagGenerator::get_instance();
	$tag_generator->add( 'checkbox', __( 'checkboxes', 'contact-form-7' ),
		'wjd_wpcf7_tag_generator_checkbox' );
	$tag_generator->add( 'radio', __( 'radio buttons', 'contact-form-7' ),
		'wjd_wpcf7_tag_generator_checkbox' );
}

function wjd_wpcf7_tag_generator_checkbox( $contact_form, $args = '' ) {
